...
export default function WorkoutPage({ params }: { params: { id: string } }) {
  ...
  const [workoutCompletionData, setWorkoutCompletionData] = useState<WorkoutCompletionData | null>(null)
  ...
  useEffect(() => {
    ...
    async function fetchWorkoutExercises() {
      try {
        ...
        const token = localStorage.getItem("token")
        const response = await axios.get(`${config.WORKOUT_URL}/${params.id}/exercises`, {
          headers: { Authorization: `Bearer ${token}` },
        })

        const formattedExercises = response.data.map(({ name, duration, rest }: Exercise) => {
          const workoutDetails = (workoutsData as WorkoutsData).workouts[name]
          return {
            name,
  ...
  const nextExerciseIndex = currentExercise + 1
  const nextExerciseData = nextExerciseIndex < exerciseDetails.length ? exerciseDetails[nextExerciseIndex] : null
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="bg-pink-100 border-none">
              <CardContent className="p-4 flex flex-row items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium">{workout.durationInMinutes}</p>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>This workout includes {exerciseDetails.length} exercises</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {exerciseDetails.map((exercise: ExerciseDetailType, index: number) => (
                <div key={index} className="flex items-center gap-4 p-3 border rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {index < exerciseDetails.length - 1 ? ` • ${exercise.rest}s rest` : ""}
                    </p>
                  </div>
                </div>
              ))}
